...
class CustomLoginView(LoginView):
    ...
    redirect_authenticated_user = False  # СТРОГО FALSE, ЧТОБЫ НЕ БЫЛО ПЕТЛИ И ГЛЮКОВ С БАЗОЙ!
    ...
    def get_success_url(self):
        ...
        # Если зашел ребенок — отправляем на /home/
...
def edit_child(request, child_id):
    ...
    if request.method == 'POST':
        if not can_edit:
            messages.error(request, 'Ошибка! Данные заблокированы.')
            return redirect('home:dashboard')
...
def delete_child(request, child_id):
    ...
    if Enrollment.objects.filter(child=child).exclude(status='rejected').exists():
        messages.error(request, f'Невозможно удалить профиль.')
        return redirect('home:dashboard')
    child.delete()
...
def generate_child_code(request):
    ...
    if not request.user.groups.filter(name='Родитель').exists():
        return JsonResponse({'error': 'Доступно только для родителей'}, status=403)
...
def child_check_code(request):
    if request.method == 'POST':
        ...
        if code_obj and code_obj.is_valid():
            ...
            messages.error(request, 'Код неверный или истек!')
